...
class DistinguishedBTests(unittest.TestCase):
    ...
    def test_common_cut_and_b_partition_options_are_keyword_safe(self):
        core_parameters = inspect.signature(SAT.core_clauses).parameters
        verify_parameters = inspect.signature(SAT.verify_model).parameters
        self.assertIn("enforce_block_pair_common_bounds", core_parameters)
        self.assertIn("b_internal_degree", core_parameters)
        self.assertIn("enforce_block_pair_common_bounds", verify_parameters)
        self.assertIn("b_internal_degree", verify_parameters)

        enabled, _ = SAT.core_clauses(
            degree=20, a_internal_degree=2, b_internal_degree=8,
            enforce_block_pair_common_bounds=True)
        disabled, _ = SAT.core_clauses(
            degree=20, a_internal_degree=2, b_internal_degree=8,
            enforce_block_pair_common_bounds=False)
        self.assertGreater(len(enabled), len(disabled))
        disabled_units = {clause[0] for clause in disabled if len(clause) == 1}
        for vertex in range(21, SAT.N):
            literal = SAT.edge_var(20, vertex)
            self.assertIn(literal if vertex <= 28 else -literal,
                          disabled_units)
